...
//PA4
#define SPI1_NSS 12
#define SPI1_NSS_PORT B
//#define START_AFR_SPI1_NSS_PA4 16

//PB3
#define SPI1_SCK 13
#define SPI1_SCK_PORT B
#define START_AFR_SPI1_SCK_PB3 20

//PA6
#define SPI1_MISO 2
#define SPI1_MISO_PORT C
#define START_AFR_SPI1_MISO_PA6 8
...
void config_SPI1()
{
    ...
    //config the pins
    RCC->AHB2ENR |= RCC_AHB1ENR_GPIOBEN;//enable clock to GPIO portA
    RCC->AHB2ENR |= RCC_AHB1ENR_GPIOCEN;//enable clock to GPIO portB
  ...
  //  GPIO_PORT(SPI1_NSS_PORT)->MODER |= (2ul<<(TWO_CONFIG_BITS(SPI1_NSS)));
   ...
   // GPIO_PORT(SPI1_NSS_PORT)->AFR[0] |= (SPI1_ARF<<START_AFR_SPI1_NSS_PA4);
    GPIO_PORT(SPI1_SCK_PORT)->AFR[1] &= ~(0xF<<START_AFR_SPI1_SCK_PB3);
    GPIO_PORT(SPI1_SCK_PORT)->AFR[1] |= (SPI1_ARF<<START_AFR_SPI1_SCK_PB3);
    ...
    GPIO_PORT(SPI1_MOSI_PORT)->AFR[1] &= ~(0xF<<START_AFR_SPI1_MOSI_PA7);
    GPIO_PORT(SPI1_MOSI_PORT)->AFR[1] |= (SPI1_ARF<<START_AFR_SPI1_MOSI_PA7);//finish setting alt functinos
    ...
    config_SPI_clock(SPI2);

    //SPI1->CR2 |= SPI_CR2_TXEIE;
    SPI2->CR2 |= SPI_CR2_RXNEIE; // recieve buffer not empty interrupt
    ...
    */
    general_config_SPI_master(SPI2);
}
